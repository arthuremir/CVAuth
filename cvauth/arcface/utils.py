...
from cvauth.arcface.data.data_pipe import de_preprocess
import torch
from cvauth.arcface.model import l2_norm
import pdb
import cv2


def get_facebank_names(path):
    return [p.stem for p in path.iterdir() if p.is_dir()]
...
def prepare_facebank(conf, model, mtcnn, tta=True):
    ...
    embeddings = torch.cat(embeddings).to(conf.device)
...
def load_facebank(conf):
    embeddings = torch.load(conf.facebank_path / 'facebank.pth', map_location=conf.device)
